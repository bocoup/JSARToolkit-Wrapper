...
FLARRasterFilter_Threshold = ASKlass('FLARRasterFilter_Threshold',
{
  ...
  ,doFilter : function(i_input, i_output)
  {
    ...
    if( window.DEBUG ){
      var debugCanvas = document.getElementById('JSARToolkit_DebugCanvas');
...
FLARParam = ASKlass('FLARParam', NyARParam,
{
  ...
  {    
    this._screen_size.w = w || 720;
    this._screen_size.h = h || 360;
    var dist = new FloatVector([w/2, h/2, 26.2, 1.1127565206658486]);
    var projection = new FloatVector([900.9514702992245, 0, w/2-0.5, 0,
                                      0, 860.0941816535367, h/2-0.5, 0,
                                      0, 0,                 1,     0]);
...
FLARSquareContourDetector = ASKlass('FLARSquareContourDetector', NyARSquareContourDetector,
{
  ...
  ,detectMarkerCB : function(i_raster ,i_callback)
  {
    ...
    for (var i=0; i < label_num; i++) {
      ...
      if (window.DEBUG) {
        var cv = document.getElementById('JSARToolkit_DebugCanvas').getContext('2d');
        cv.strokeStyle = 'red';
...
PerspectivePixelReader = ASKlass('PerspectivePixelReader',
{
  ...
  */
  getRowFrequency : function(i_reader,i_raster_size,i_y1,i_th_h,i_th_l,o_edge_index)
  {
    ...
    var cv;
    if (window.DEBUG) {
      cv = document.getElementById('JSARToolkit_DebugCanvas').getContext('2d');
      cv.fillStyle = 'orange';
    }
